...
get_pib_sectores <- function(
  ...
  # nolint start
  url_2018 <- "https://cdn.bancentral.gov.do/documents/estadisticas/sector-real/documents/pib_origen_2018.xlsx"
  url_retro <- "https://cdn.bancentral.gov.do/documents/estadisticas/sector-real/documents/pib_origen_retro_2018.xlsx"
  # nolint end

  temp_file <- tempfile(fileext = ".xlsx")
  url <- if (homogenea_91) url_retro else url_2018
  start_date <- if (homogenea_91) "1991-01-01" else "2018-01-01"

  utils::download.file(url, temp_file, mode = "wb", quiet = TRUE)

  sheet <- dplyr::case_when(
    modalidad == "nominal" & acumulado  & homogenea_91 ~ "PIB$_Trim Acum",
    modalidad == "real"    & acumulado  & homogenea_91 ~ "PIBK_Trim Acum",
  ...
  to_read <- list(
    nominal = list(
      list(skip = skips$nominal[1], nmax = nmax, indicator_name = "pib_nominal"),
      list(skip = skips$nominal[2], nmax = nmax, indicator_name = "ponderacion")
    ),
    real = list(
      list(skip = skips$real[1], nmax = nmax, indicator_name = "indice"),
      list(skip = skips$real[2], nmax = nmax, indicator_name = "crecimiento_interanual"),
      list(skip = skips$real[3], nmax = nmax, indicator_name = "incidencia")
    )
  )
...
#' @return a data frame
#' @export
get_pib_sectores_2007 <- function(
    modalidad    = "real",
    acumulado    = FALSE,
    homogenea_91 = FALSE
) {
